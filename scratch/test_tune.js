...
// Let's test a refined formulation:
// Keep Pmus ramp over 0.70 * tiN with smooth sinusoidal / linear blend:
simCode = simCode.replace(
`        if (tn < 0.75 * tiN) {
            // Lineær opptrapping
            pmus = (0.75 * tiN > 0) ? pMax * (tn / (0.75 * tiN)) : 0;
        } else if (tn < tiN) {`,
`        const tRiseN = 0.70 * tiN;
        if (tn < tRiseN) {
            // Fysiologisk opptrapping av nevromuskulær drivkurve (Pmus)
...
`        // Fysiologisk/pneumatisk ventilregulering under stigetid:
        ...
        const riseFactor = (this.state.phase === 'inspiration')
            ...
            : 0;
        const starvationScale = 1.0 + 5.0 * riseFactor * (this.state.P_mus > 0 ? Math.min(1.0, this.state.P_mus / 3.0) : 0);
...
fs.writeFileSync('scratch/simulator_tuned.js', simCode);

function runSuite(testFile) {
    console.log(`\n================== RUNNING ${testFile} ==================`);
    let testCode = fs.readFileSync(testFile, 'utf8');
    testCode = testCode.replace(/path\.join\(__dirname,\s*['"]simulator\.js['"]\)/g, "path.join(__dirname, 'scratch', 'simulator_tuned.js')");
    testCode = testCode.replace(/__dirname\s*\+\s*['"]\/simulator\.js['"]/g, "path.join(__dirname, 'scratch', 'simulator_tuned.js')");
    testCode = testCode.replace(/fs\.readFileSync\(['"]simulator\.js['"],\s*['"]utf8['"]\)/g, "fs.readFileSync(path.join(__dirname, 'scratch', 'simulator_tuned.js'), 'utf8')");
    testCode = testCode.replace(/fs\.readFileSync\(['"]\.\/simulator\.js['"],\s*['"]utf8['"]\)/g, "fs.readFileSync(path.join(__dirname, 'scratch', 'simulator_tuned.js'), 'utf8')");
